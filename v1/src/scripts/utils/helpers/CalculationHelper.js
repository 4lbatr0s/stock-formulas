...
class CalculationHelper{
    ...
    grahamNumber(stockJson){
        ...
        if(!epsTrailingExistsAndNumber || !bookValueExistAndNumber)
            return '';
        return Math.sqrt(stockJson.epsTrailingTwelveMonths * stockJson.bookValue * 22.5);
    }
    /**
     * 
     * @param {Object} stockJson JSON object of the stock we want make calculation from.
    ...
    priceToEarningRate(stockJson){
        const epsTrailingExistsAndNumber = stockJson?.epsTrailingTwelveMonths && typeof stockJson.epsTrailingTwelveMonths === 'number';
        const regularMarketPriceExistsAndNumber =  stockJson?.regularMarketPrice && typeof stockJson.regularMarketPrice === 'number';
        if(!epsTrailingExistsAndNumber || !regularMarketPriceExistsAndNumber)
            return '';
        return Math.fround(stockJson.regularMarketPrice/stockJson.epsTrailingTwelveMonths);
    }
}
