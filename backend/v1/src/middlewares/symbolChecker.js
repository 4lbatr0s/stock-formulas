...
const symbolChecker = (req, res, next) => {
  ...
  if (!symbolField) { return next(new ApiError(Messages.ERROR.STOCK_SYMBOL_REQUIRED, httpStatus.BAD_REQUEST)); }
  // return next(globalErrorHandler(Messages.ERROR.STOCK_SYMBOL_REQUIRED));
  if (!symbolField.match(/[A-Za-z]/)) { return next(new ApiError(Messages.ERROR.BAD_STOCK_SYMBOL, httpStatus.BAD_REQUEST)); }
  // return next(globalErrorHandler(Messages.ERROR.BAD_STOCK_SYMBOL));
  if (!(stockSymbols.includes(symbolField))) { return next(new ApiError(Messages.ERROR.STOCK_SYMBOL_NOT_FOUND, httpStatus.NOT_FOUND)); }
